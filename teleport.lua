...
function Teleport:Smart(...)
	...
	if (params[3] == nil) then return print("You need a CFrame or Instance to teleport to.") end
	if (typeof(params[3]) == "CFrame") then
		if (Teleport:Distance((params[3] + Vector3.new(0, 5, 0))) <= Teleport.Settings.MinimumInstant and Teleport:Distance((params[3] + Vector3.new(0, 5, 0))) <= Teleport.Settings.MaximumInstantTeleport) then
			lp.Character.HumanoidRootPart.CFrame = (params[3] + Vector3.new(0, 5, 0)); return
		end
		...
		Teleport.TweenAnim:Play(); Teleport.TweenAnim.Completed:Wait(); Teleport.TweenAnim = nil
	elseif (typeof(params[3]) == "Instance") then
		if (Teleport:Distance((params[3].CFrame + Vector3.new(0, 5, 0))) <= Teleport.Settings.MinimumInstant and Teleport:Distance((params[3].CFrame + Vector3.new(0, 5, 0))) <= Teleport.Settings.MaximumInstantTeleport) then
			lp.Character.HumanoidRootPart.CFrame = (params[3].CFrame + Vector3.new(0, 5, 0)); return
		end
